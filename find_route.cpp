...
static route_ptr head;
extern city cities[CITY_NUM];

void trace_back(route_ptr cur_ptr, const request & user_request, double risk_v, int time_v, route_info & best_route);
void dulplicate_best_route(route_info & best_route, route_ptr & temp_route);
extern void get_time(my_time &);
extern void add_time(my_time &, int);

void find_route(const request & user_request, route_info & best_route){
	...
	trace_back(head, user_request, 0, 0, best_route);
}
...
void trace_back(route_ptr cur_ptr, const request & user_request, double risk_v, int time_v, route_info & best_route){
	...
	for (i = cities[cur_ptr->city_no].my_vehicle.begin(); i != j; i++){
		...
		if (i->dst_city_no==user_request.dst_city_no){
			...
			trace_back(new_route, user_request, risk_v, time_v, best_route);
			delete new_route;
